...
function QuickAccessItem({ title, imageLink, isLarge = false, titleInsideCard = false, colSpan = 1 }) {
  ...
  // Base card styling
  let cardBaseClasses =
    "bg-gray-100 rounded-2xl w-full overflow-hidden transition-transform hover:scale-[1.02] active:scale-98 cursor-pointer";
  ...
  if (titleInsideCard && isLarge) {
    ...
    imageClasses = "h-30 w-30 object-contain self-end";
  } else {
  ...
  return (
    <div className="w-full max-w-md space-y-6">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-6">
        ...
        <QuickAccessItem title={items[1].title} imageLink={items[1].imageLink} colSpan={1} />
        <QuickAccessItem title={items[2].title} imageLink={items[2].imageLink} colSpan={1} />

        {/* Items 4 & 5: "Transit" & "Charter" */}
        <QuickAccessItem title={items[3].title} imageLink={items[3].imageLink} colSpan={1} />
        <QuickAccessItem title={items[4].title} imageLink={items[4].imageLink} colSpan={1} />
      </div>
    </div>
  );
}
