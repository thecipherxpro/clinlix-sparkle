...
export const JobCard: React.FC<JobCardProps> = ({
  ...
  return <article onClick={onClick} className={cn("relative w-full bg-card rounded-2xl p-4 sm:p-6 min-h-[140px] sm:min-h-[160px]", "transition-all duration-300 hover:shadow-lg hover:scale-[1.02]", "flex flex-col justify-between", onClick && "cursor-pointer active:scale-[0.98]", className)} style={{
    backgroundColor: heroColor
      ...
      {/* Arrow Button */}
      <div className="absolute right-3 bottom-3 w-7 h-7 sm:w-7 sm:h-7 bg-foreground rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-transform touch-target">
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-background translate-x-0.5">
