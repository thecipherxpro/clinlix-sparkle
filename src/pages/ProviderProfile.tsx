      ...
      <main className="mobile-container py-4 sm:py-8 max-w-4xl pb-24">
        ...
        <div className="relative mb-6 sm:mb-8">
          ...
          <div className="absolute -bottom-12 sm:-bottom-16 left-4 sm:left-8">
            <AvatarDisplay userId={provider.user_id} avatarUrl={provider.photo_url} size={window.innerWidth >= 640 ? 128 : 96} fallbackText={provider.full_name.split(" ").map((n: string) => n[0]).join("")} className="border-4 border-background rounded-full" />
          </div>
        </div>
        ...
        <div className="mt-16 sm:mt-20">
          ...
          {provider.skills && provider.skills.length > 0 && <div className="flex flex-wrap gap-2 mb-4 sm:mb-6">
              ...
              {provider.service_areas && provider.service_areas.length > 0 && <Card className="border-0 shadow-sm">
                  <CardContent className="pt-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {provider.service_areas.map((area: string, idx: number) => <div key={idx} className="badge badge-accent px-[17px]">
                          {area}
                        </div>)}
                    </div>
                  </CardContent>
